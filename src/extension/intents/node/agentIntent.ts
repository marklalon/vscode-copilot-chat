...
export class AgentIntent extends EditCodeIntent {
	...
	private async handleSummarizeCommand(
		...
		const compactResolution = await this.mem0SmartCompactService.resolveCompactEndpoint(endpoint);
		const compactEndpoint = compactResolution.endpoint;
		...
		try {
			...
			const chatResult: vscode.ChatResult = {
				...(compactResolution.details ? { details: compactResolution.details } : {}),
				metadata: {
					summary: {
...
export class AgentIntentInvocation extends EditCodeIntentInvocation implements IIntentInvocation {
	...
	private _startBackgroundSummarization(
		...
		const bgRenderer = PromptRenderer.create(this.instantiationService, compactEndpoint, this.prompt, {
			...snapshotProps,
			endpoint: compactEndpoint,
